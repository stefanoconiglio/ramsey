...
def extract_terminal_status(line: str) -> str | None:
    match = re.search(r"\b(FACET|FAILURE)\s*$", line)
    return match.group(1) if match else None


def replace_terminal_status(line: str, new_status: str) -> str:
    return re.sub(r"\b(FACET|FAILURE)\s*$", new_status, line)
...
def parse_args() -> argparse.Namespace:
    ...
    lift_group.add_argument(
        ...
        action="store_const",
        const="lifted",
        help="Run only the lifted inequality (forwards --lifted).",
    )
    lift_group.add_argument(
        "--addlifted",
        dest="lift_mode",
        action="store_const",
        const="addlifted",
        help="For each case, run the standard inequality first and then the lifted one.",
    )
    lift_group.add_argument(
        "--lift",
        dest="lift_mode",
...
def main() -> None:
    ...
    for t, s_raw, m in combos:
        def build_cmd(use_lifted: bool) -> list[str]:
            ...
            if args.output_dir is not None:
                suffix = "_lift" if use_lifted else ""
                out_path = args.output_dir / f"S={s_raw}_m={m}_t={t}{suffix}.tex"
                cmd.extend(["--output", str(out_path)])
            return cmd

        if args.lift_mode == "addlifted":
            standard_rc, standard_line = run_and_extract_one_line(build_cmd(False))
            lifted_rc, lifted_line = run_and_extract_one_line(build_cmd(True))

            standard_status = extract_terminal_status(standard_line)
            lifted_status = extract_terminal_status(lifted_line)
            if standard_status != "FACET" and lifted_status == "FACET":
                lifted_line = replace_terminal_status(lifted_line, "FACET+")

            if standard_line:
                print(standard_line)
            else:
                print(f"t={t}, S={{{s_raw}}}, m={m}, EXIT={standard_rc}")

            if lifted_line:
                print(lifted_line)
            else:
                print(f"t={t}, S={{{s_raw}}}, m={m}, EXIT={lifted_rc}")

            for rc in (standard_rc, lifted_rc):
                if rc != 0:
                    failures += 1
        ...
        use_lifted = args.lift_mode == "lifted"
        rc, one_line = run_and_extract_one_line(build_cmd(use_lifted))
        if one_line:
            print(one_line)
        else:
