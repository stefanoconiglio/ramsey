...
2^|J|-1. For each graph G_L, where L={l in J : y_l=1}, the script reports:
  - which jumps are used (L),
  - the indicator vector y_l (1 if l in L, else 0),
  - |E(L) \\cap E(S)| and whether it equals d_m(|S|).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(whether there exists y in {0,1}^|J| with a'y=d_m(|S|)). "
...
def final_status_line(
    ...
    return (
        f"{current}/{total} "
        f"t={t}, m={m}, thm2={thm2}, S={plain_set(s_sorted)}, floor(t/2)={j_max}, "
        f"N={n_check}, rank(Y+)={rank_y_plus}, alldiff={alldiff}, d_m(|S|)={d_m_s}, a'y=({coeffs_str})'y, "
        f"{uniformity}, SSSP={sssp}, {facet_status(rank_y_plus, j_max)}"
    )
...
def compute_summary(
    ...
    a_coeffs = [0 for _ in j_list]
    for i, j in s_edges:
        jump = jump_distance(i, j, t)
        a_coeffs[jump_to_idx[jump]] += 1
    jump_overlap_count = {jump: a_coeffs[idx] for idx, jump in enumerate(j_list)}
    sssp = sssp_value(a_coeffs, d_m_s, compute_sssp_exact)
    graph_data: list[tuple[int, set[int], set[tuple[int, int]], int, bool, bool]] = []
    last_status_len = 0
    if use_recipe:
        ...
        for graph_idx, jumps_l in enumerate(recipe_subsets, start=1):
            ...
            mark_ok = overlap_count == d_m_s
            is_minimal = mark_ok and all(jump_overlap_count.get(jump, 0) > 0 for jump in jumps_l)
            graph_data.append((graph_idx, jumps_l, edges_l, overlap_count, mark_ok, is_minimal))
        ...
        selected_graphs = [entry for entry in graph_data if entry[4]]

    check_indices = [entry[0] for entry in selected_graphs]
    check_y_columns = [jump_y_vector(j_list, entry[1]) for entry in selected_graphs]
...
def complete_graph_highlight_s_tex(
    ...
    s_edges = edges_from_vertex_subset(s_set)
    s_size = len(s_set)
    edges_with_jumps = sorted(
        ((jump_distance(i, j, t), i, j) for i, j in s_edges),
        key=lambda item: (item[0], item[1], item[2]),
    )
    s_jumps = sorted({jump for jump, _, _ in edges_with_jumps})
    jump_counts: dict[int, int] = {}
    for jump, _, _ in edges_with_jumps:
        jump_counts[jump] = jump_counts.get(jump, 0) + 1
    edge_terms = " + ".join(f"b_{{{i}{j}}}" for _, i, j in edges_with_jumps)
    if not edge_terms:
        edge_terms = "0"
    jump_terms_parts: list[str] = []
    for jump in sorted(jump_counts):
        coeff = jump_counts[jump]
        if coeff == 1:
            jump_terms_parts.append(rf"y_{{{jump}}}")
        else:
            jump_terms_parts.append(rf"{coeff} y_{{{jump}}}")
    jump_terms = " + ".join(jump_terms_parts) if jump_terms_parts else "0"
    lines: list[str] = []
    lines.append(
    ...
    lines.append(
        rf"\par ${edge_terms} \le d_m(|S|)=d_{{{m}}}({s_size})={d_m_s}$"
    )
    lines.append(rf"\par ${jump_terms} \le {d_m_s}$")
    return "\n".join(lines)
...
def graph_cell_tex(
    ...
    lines.append(rf"\par $|E(L)\cap E(S)|={overlap_with_s}\;\;{mark_symbol}$")
    lines.append(r"\end{minipage}")
    return "\n".join(lines)
...
def build_document(summary: dict, cols: int, show_all_graphs: bool) -> str:
    ...
    if use_recipe:
        ...
        display_graphs = [entry for entry in graph_data if entry[4]]
    ...
    doc.append(r"\begin{center}")
    doc.append(complete_graph_highlight_s_tex(t=t, s_set=s_set, m=m, d_m_s=d_m_s))
    doc.append(r"\end{center}")
    doc.append(r"")
    ...
    if not display_graphs:
        ...
        for start in range(0, len(display_graphs), cols):
            ...
            doc.append(r"\noindent")
            row_cells: list[str] = []
            for graph_idx, jumps_l, edges_l, overlap_count, mark_ok, is_minimal in row_subsets:
                row_cells.append(
                    graph_cell_tex(
                        idx=graph_idx,
                        t=t,
...
def default_output_path(t: int, s_set: set[int], m: int) -> Path:
    s_part = ",".join(str(v) for v in sorted(s_set)) if s_set else "empty"
    return Path(f"S={s_part}_m={m}_t={t}.tex")
...
def main() -> None:
    ...
    output = args.output if args.output is not None else default_output_path(args.t, s_set, m)
